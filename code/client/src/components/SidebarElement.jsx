...
function SidebarElement({ text, icon, path, isItemActive }) {
	...
	useEffect(() => {
		location.pathname.startsWith('/coding-page')
			? setMessage("Your progress will be automatically saved")
			: setMessage("Your progress will be lost");
	}, []);

	const handleNavigation = (event) => {
		if (location.pathname.startsWith('/coding-page') || location.pathname.startsWith('/propose-page')) {
	...
	return (
		<li>
			<Link
				to={path}
				className={`flex items-center py-2 border pl-3 group ${isItemActive(path)
